...
def check_program_syntax(program):
    ''' Takes a postfix program as a string, verifies that it has the correct
    syntax and returns a boolean representing whether it's valid. If it is
    valid, it also returns an int which is the number of required args
    ...
    for token in token_list[2:]:
        if token not in [valid_commands, parens] and not is_int(token):
...
def check_arg_syntax(args):
    ''' Given a string, verifies that every word in the string can be converted
    to an integer, and returns the number of arguments encountered
...
def run_program(program, arguments):
    ...
    prog = [convert_int(x) for x in prog]
    prog, _ = tokenize(prog, 0)

    # put args on stack
    args = [int(x) for x in arguments.split()]
    args.reverse()
...
def main():
    ...
    if interactive:
        ...
        prog7 = '(postfix 1 ((3 nget swap exec) '\
                '(2 mul swap exec) swap) (5 sub) swap exec exec)'
        print(prog7)
